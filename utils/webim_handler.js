    ...
    cameraContext.takePhoto({
      ...
      success: function (res) {
        ...
        const uploadTask = wx.uploadFile({
          url: 'https://survey.zhongchebaolian.com/boot-zcbl-survey-api/survey/weixin/v1/upload', //仅为示例，非真实的接口地址
          filePath: res.tempImagePath,
          name: 'file',
          formData: {
            'user': 'test'
...
//  }
 ...
 function onMsgNotify(newMsgList) {
   ...
   for (var j in newMsgList) {//遍历新消息
     newMsg = newMsgList[j];
     var selToID = newMsg.fromAccount;
     selSess = newMsg.getSession();
     //在聊天窗体中新增一条消息
      dataString = convertMsgtoHtml(newMsg);
     console.log('收到一条新消息 50===> ' + dataString);
     dataString = dataString.replace(/&quot;/g, "'");
     console.log('收到一条新消息 44===> ' + dataString);
     if (dataString == 0){
       wx.navigateBack({
         delta: 1
       })
     }
     if (dataString == 'true') {//拍照
       takePhone()
     }
...
function convertMsgtoHtml(msg) {
    ...
    for (var i in elems) {
        ...
        console.log('305'+type)
        console.log(webim.MSG_ELEMENT_TYPE.TEXT)
 ...
 */
//发送自定义消息
function sendCustomMsgtext(obj) {
  console.log(obj)
  var selType = webim.SESSION_TYPE.C2C;//一对一
  var selToID = 'anonymous';//用户ID
  var selSessHeadUrl;
  ...
  var text_obj = new webim.Msg.Elem.Text(obj);
  msg.addText(text_obj);
...
module.exports = {
  takePhone: takePhone,
    ...
    sendCustomMsgtext: sendCustomMsgtext,
